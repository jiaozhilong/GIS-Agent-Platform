...
CREATE TABLE IF NOT EXISTS llm_providers (
    ...
    model            VARCHAR(128),                   -- 模型名，如 deepseek-chat / gpt-4o / qwen-max
...
-- ============================================================
-- IMA 用户凭证（按用户隔离，加密存储；与 ima_kb_configs 一对多：一用户一套 IMA 账号 + 多知识库）
-- ============================================================
CREATE TABLE IF NOT EXISTS ima_credentials (
    id                  BIGSERIAL PRIMARY KEY,
    user_id             BIGINT       NOT NULL UNIQUE REFERENCES users(id) ON DELETE CASCADE,
    client_id_encrypted VARCHAR(512),
...
CREATE INDEX IF NOT EXISTS idx_ima_kb_configs_user ON ima_kb_configs(user_id);
CREATE INDEX IF NOT EXISTS idx_ima_kb_configs_purpose ON ima_kb_configs(user_id, purpose);
CREATE INDEX IF NOT EXISTS idx_projects_user ON projects(user_id);
CREATE INDEX IF NOT EXISTS idx_projects_status ON projects(status);
CREATE INDEX IF NOT EXISTS idx_pipeline_runs_project ON pipeline_runs(project_id);
CREATE INDEX IF NOT EXISTS idx_tool_executions_pipeline ON tool_executions(pipeline_run_id);
CREATE INDEX IF NOT EXISTS idx_exports_project ON exports(project_id);

-- ============================================================
-- 更新时间维护说明
-- updated_at 由各实体的 JPA @PreUpdate 钩子维护（见 User/Project/LlmProvider/ImaKbConfig），
-- 故此处不再建数据库触发器，避免 PL/pgSQL 函数体与 Spring sql.init 默认 ';' 分隔符冲突。
-- ============================================================
...
-- ============================================================
-- 预置流程模板（模板市场底座）。注意：本表早期版本漏写在 schema 中，仅留 ALTER 追加列，
-- 导致全新库启动失败；现 owner_id/like_count/favorite_count/published 一并纳入建表语句，
-- 保证全新库一次成型（ALTER 形式已移除，避免引用尚未存在的表）。
CREATE TABLE IF NOT EXISTS pipeline_templates (
    id              BIGSERIAL PRIMARY KEY,
    template_key    VARCHAR(64)  NOT NULL UNIQUE,        -- 模板唯一键，如 quick_selection / full_solution
    name            VARCHAR(128) NOT NULL,
    category        VARCHAR(32),                         -- official | community | mine
    description     TEXT,
    tool_chain      JSONB,                               -- 工具节点序列 JSON 数组（toolType 列表）
    estimated_time  VARCHAR(32),
    usage_count     BIGINT       NOT NULL DEFAULT 0,
    owner_id        BIGINT,                              -- 作者用户 ID（官方模板为 null）
    like_count      BIGINT       NOT NULL DEFAULT 0,
    favorite_count  BIGINT       NOT NULL DEFAULT 0,
    published       BOOLEAN      NOT NULL DEFAULT TRUE,  -- 是否上架
    builtin         BOOLEAN      NOT NULL DEFAULT FALSE, -- 是否内置官方模板
    created_at      TIMESTAMP    NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMP    NOT NULL DEFAULT NOW()
);
